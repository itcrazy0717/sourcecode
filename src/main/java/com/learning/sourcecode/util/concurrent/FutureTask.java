...
public class FutureTask<V> implements RunnableFuture<V> {
    ...
    private V report(int s) throws ExecutionException {
        Object x = outcome;
        // 如果FutureTask的状态正常，则直接返回值
        if (s == NORMAL)
            return (V)x;
        // 如果任务被取消，抛出CancellationException异常
        if (s >= CANCELLED)
            throw new CancellationException();
        // 否则抛出ExecutionException异常
    ...
    protected void set(V v) {
        // 通过cas更改FutureTask的状态值，修改为COMPLETING
    ...
    // 未用锁来保证并发，而是使用cas来保证并发安全，保证只有一个线程能运行FutureTask任务
    public void run() {
        /**
         * 如果state不是NEW，或者设置Runner值失败
         * 表示有别的线程在此之前调用了run方法，并成功设置了runner的值
         * 其实就是设置当前调用线程到runner上
    ...
    private int awaitDone(boolean timed, long nanos)
        ...
        for (;;) {
            ...
            if (Thread.interrupted()) {
                // 从列表中移除节点，并抛出InterruptedException异常
                removeWaiter(q);
