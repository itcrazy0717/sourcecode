...
public class ArrayList<E> extends AbstractList<E>
    ...
    public ArrayList(Collection<? extends E> c) {
        // 接收一个Collection类型的参数作为初始元素
        // 转换成数组
        elementData = c.toArray();
        if ((size = elementData.length) != 0) {
            // c.toArray might (incorrectly) not return Object[] (see 6260652)
            // 存在子类重写父类，所以c.toArray返回值不一定是Object，比如先声明为String[]，后又将其赋值为Object则会报错
    ...
    public E remove(int index) {
        ...
        // 大于0，则不在最后一位
        if (numMoved > 0)
            // 则从index+1位置向前移动一位，则去除index位置的数组
    ...
    protected void removeRange(int fromIndex, int toIndex) {
        ...
        // 从toIndex位置移动numMoved到elementData(fromIndex位置开始)
        System.arraycopy(elementData, toIndex, elementData, fromIndex,
                         numMoved);

        // clear to let GC do its work
        // 获取需要GC的元素开始处 size-toIndex+fromIndex
    ...
    /**
     * 批量移除元素 complement-true，则覆盖，false
     * @param c
     * @param complement
     * @return
     */
    private boolean batchRemove(Collection<?> c, boolean complement) {
        ...
        try {
            // 遍历
            for (; r < size; r++)
               // 判断c中是否包含elementData中的元素  
                // 如果complement为false 则表示求差集，保留ArrayList中在c中不存在的元素 removeAll
                // 如果complement为true，则表示求交集，保留ArrayList中和C中共同存在的元素 retainAll
