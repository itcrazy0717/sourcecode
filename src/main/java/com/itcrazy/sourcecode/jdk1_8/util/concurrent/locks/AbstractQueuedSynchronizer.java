...
public abstract class AbstractQueuedSynchronizer
    ...
    private Node addWaiter(Node mode) {
        ...
        // 当然第一次进来tail、head节点都为null
        // 如果再有线程进来，此时tail就不为空了
        // 通过cas操作将线程节点构建在尾部
        Node pred = tail;
        if (pred != null) {
            node.prev = pred;
        ...
        // 构建双向链表 enq的作用就是构建双向链表，将线程节点直接插入到链表尾部
        enq(node);
        return node;
    }
    ...
    private void unparkSuccessor(Node node) {
         ...
         */
        // 在shouldParkAfterFailedAcquire中将线程的waitStatus更新为了SINGAL(-1)
        // 获取head节点的状态
        ...
        // head节点的下一个节点
        Node s = node.next;
        // 这里从tail向前找，找到距离head最近的一个waitStatus<=0的节点
        // 从后往前轮询，保证链表不中断，具体在enq方法构建双向链表时，从后往前，保证轮询不中断
        //  如果s==null或者s.waitStatus>0(只有CANCELLED=1)，表示有节点线程超时或被中断了
        ...
        // 唤醒挂起的线程，注意挂起的线程目前仍然在自旋处acquireQueued函数中
        if (s != null)
    ...
    /**
     * 该方法的主要作用就是判断线程是否应该被挂起
     * 也就是将节点的waitStatus状态更新为SIGNAL
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws == Node.SIGNAL)
            ...
            // 返回true，表示可以挂起当前线程
            return true;
        // waitStatus状态中只有CANCELLED(1)>0 表示线程超时或中断  只需要移除该节点即可   
        if (ws > 0) {
            /*
             * Predecessor was cancelled. Skip over predecessors and
             * indicate retry.
             */
            // 清除waitStatus状态为CANCELLED的点
            /**
             * pred=pred.prev
             * node.prev=pred
             * 这里就相当于移除了pred节点
             ...
             */
            // 通过cas操作将节点的waitStatus更新为SIGNAL(-1)
    ...
    /**
     * 挂起当前线程，这里还不会返回的，只有等待线程被唤醒后，才会返回
    ...
    private final boolean parkAndCheckInterrupt() {
        ...
        // Thread.interrupted()方法判断中断状态，并复位
        // 这里是为了线程在获取锁的过程中，不能及时响应中断，因此需要返回中断标记
        // 进一步对中断做出响应 如果返回true，则会在acqurire中会执行selfInterrupt(进行线程中断打标)
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        public final void await() throws InterruptedException {
            ...
            // 当线程醒来就会尝试获取锁，如果acquireQueued返回false，就表示获取锁成功，因为这里返回的是中断状态
            // 如果被中断会抛出异常
            if (acquireQueued(node, savedState) && interruptMode != THROW_IE)
                interruptMode = REINTERRUPT;
            // 如果node的下一个等待节点不是null，则进行清理
