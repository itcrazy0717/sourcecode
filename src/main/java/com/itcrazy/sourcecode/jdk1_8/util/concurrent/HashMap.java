    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // i位置上有元素
            ...
            if (e != null) { // existing mapping for key
                V oldValue = e.value;
                // onlyIfAbsent表示是否要改变原来的值，true-不改变，false-改变，true时，还要判断oldValue是否为空
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    // 当走到这里，说明节点上为链表形式存储数据，需进行循环操作
                    else { // preserve order
                        // 存储位置在newTable和oldTable位置不变元素 低位链表
                        Node<K,V> loHead = null, loTail = null;
                        // 存储oldTable中位置发生了变化的元素，当然这里是和oldTable相比较 高位链表
