 ...
 */
package com.itcrazy.sourcecode.redis;
...
//        get(lockAsync(leaseTime, unit));
    ...
    <T> RFuture<T> tryLockInnerAsync(long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
	    ...
	    /**
	     *  KEYS[1]->Collections.<Object>singletonList(getName()) 锁名称
	     *  ARGV[1]->internalLockLeaseTime 过期时间
	     *  ARGV[2]->getLockName(threadId) 锁唯一标识
 	     */
	    return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE, command,
                  // 如果key不存在，则设置key
                  // 注意hincrby是hash命令，用于将指定字段增加增量值
                  // 此处的意思就是如果key不存在，则使用hash结构进行存储，filed为线程id，值为1
                  // key-锁名称（KEYS[1]） filed-锁唯一标识（ARGV[2]） 设置值为1
                  "if (redis.call('exists', KEYS[1]) == 0) then " +
                      "redis.call('hincrby', KEYS[1], ARGV[2], 1); " +
                      "redis.call('pexpire', KEYS[1], ARGV[1]); " +
                      "return nil; " +
                  "end; " +
                  // 如果hash中对应的值存，则将hash中filed对应的值增加1，实现可重入
                  // 可重入的时候，重新设置了过期时间
                  "if (redis.call('hexists', KEYS[1], ARGV[2]) == 1) then " +
                      "redis.call('hincrby', KEYS[1], ARGV[2], 1); " +
                      "redis.call('pexpire', KEYS[1], ARGV[1]); " +
